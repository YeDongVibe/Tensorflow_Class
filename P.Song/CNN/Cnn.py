...
model.add(layers.Conv2D(32, (3, 3), activation=act, input_shape = (28, 28, 1))) # 2차원 convolution적용., 흑백이라서 1임. color면 일반적으로 3
model.add(layers.MaxPooling2D((2, 2))) #pooling을 2*2로 지정
...
ret = model.fit(x_train, y_train, epochs=5, batch_size=64, validation_data=(x_test, y_test), verbose=0)
